...
import { SquadDto, SquadDtoState } from "proto/maptool/phobos.maptool.squad";
import { Squad, SquadState } from "src/core/common/models/squad";

@Injectable()
export class SquadDtoService {
    public fromDto(dto: SquadDto): Squad {
        return {
            name: dto.name,
            position: dto.position,
            callsign: dto.callsign,
            combattants: dto.combattants,
            state: this.fromState(dto.state)
        }
    }

    public toDto(squad: Squad): SquadDto {
        return {
            name: squad.name,
            position: squad.position,
            callsign: squad.callsign,
            combattants: squad.combattants,
            state: this.toState(squad.state)
        }
    }

    private fromState(state: SquadDtoState): SquadState {
        switch (state) {
            case 0:
    ...
    private toState(state: SquadState): SquadDtoState {
        switch (state) {
            case 0:
